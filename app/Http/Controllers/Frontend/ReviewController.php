...
class ReviewController extends Controller
{
    ...
    {
        $product = Product::where('id', $product_slug)->where('status', '0')->first();
        if($product)
        {
            $product_id = $product->id;
            $review = Review::where('user_id', Auth::id())->where('prod_id',$product_id)->first();
    ...
    public function update(Request $request)
    {
        ...
        if($user_review !='')
        {
            ...
            if($review)
            {
                ...
                return redirect('category/'.$review->product->category->slug.'/'.$review->product->slug)->with('status', "Review Updated");
            }
            else
            {
                return redirect()->back()->with('status', "The link you followed was broken");
